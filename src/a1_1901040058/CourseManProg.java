...
public class CourseManProg {
    public static void main(String[] args) throws Exception {
        ...
        ElectiveModule cgr = new ElectiveModule("CGR", 1, 3, "FIT");
        ...
        Enrolment dongnvWprEnrolment = new Enrolment(dong_nv, wpr, 0, 0);
        manager.addEnrolment(dongnvWprEnrolment);
        manager.setMarks(dong_nv, wpr, 9.2, 7.8);
        manager.getEnrolment(dong_nv, wpr);
        ...
        Enrolment phuocntReqEnrolment = new Enrolment(phuocnt, req, 0, 0);
        manager.addEnrolment(phuocntReqEnrolment);
        manager.setMarks(phuocnt, req, 9, 6);
        manager.getEnrolment(phuocnt, req);

        Enrolment phuocNtDbsEnrolment = new Enrolment(phuocnt, dbs, (float) 8.5, 10);

        Enrolment nghiantCgrEnrolment = new Enrolment(nghiant, cgr, 0, 0);
        manager.addEnrolment(nghiantCgrEnrolment);
        manager.setMarks(nghiant, cgr, 8, 9);
        manager.getEnrolment(nghiant, cgr);
        ...
        Enrolment chienDqDbsEnrolment = new Enrolment(chiendq, dbs, 0, 0);
        manager.addEnrolment(chienDqDbsEnrolment);
        manager.setMarks(chiendq, dbs, 8, 7.5);
        manager.getEnrolment(chiendq, dbs);

        Enrolment chienDqSs1Enrolment = new Enrolment(chiendq, ss1, 10, 10);

        Enrolment thanhlmSqaEnrolment = new Enrolment(thanhlm, sqa, 0, 0);
        manager.addEnrolment(thanhlmSqaEnrolment);
        manager.setMarks(thanhlm, sqa, 8.5, 6.5);
        manager.getEnrolment(thanhlm, sqa);
